...
add_to_window_list (rp_screen *s, Window w)
{
  ...
  if (group)
    ...
    group_add_window (rp_current_group[s->screen_num], new_window);
...
{
  return group_last_window (rp_current_group[screen->screen_num], screen);
}
                 ...
                 int *mark_start, int *mark_end)
{
  ...
  list_for_each_entry (we,&rp_current_group[current_screen()->screen_num]->mapped_windows,node)
    {
      ...
      /* Only put the delimiter between the windows, and not after the the last
         window. */
      if (delim && we->node.next != &rp_current_group[current_screen()->screen_num]->mapped_windows)
