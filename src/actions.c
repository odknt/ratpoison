...
cmd_prev (int interactive UNUSED, struct cmdarg **args UNUSED)
{
  rp_window *cur, *win;
  cur = current_window();
  win = group_prev_window (rp_current_group[current_screen()->screen_num], cur);
...
cmd_next (int interactive UNUSED, struct cmdarg **args UNUSED)
{
  rp_window *cur, *win;
  cur = current_window();
  win = group_next_window (rp_current_group[current_screen()->screen_num], cur);
...
/*   w = find_window_other (); */
  w = group_last_window (rp_current_group[current_screen()->screen_num], current_screen());
...
window_completions (char* str UNUSED)
{
  ...
  list_for_each_entry (cur, &rp_current_group[current_screen()->screen_num]->mapped_windows, node)
    {
      struct sbuf *name;
  ...
  if (strlen (str) > 0)
    {
      ...
      /* try by number */
        ...
        {
          rp_window_elem *elem = group_find_window_by_number (rp_current_group[current_screen()->screen_num], n);
...
group_completions (char *str UNUSED)
{
  ...
  list_for_each_entry (cur, &rp_groups[current_screen()->screen_num], node)
    {
      struct sbuf *s;
...
/*   wait((int *) 0); */
  ...
  child->terminated = 0;
  child->frame = frame;
  child->group = rp_current_group[current_screen()->screen_num];
...
cmd_number (int interactive UNUSED, struct cmdarg **args)
{
  ...
  if (args[1])
    ...
    (rp_current_group[current_screen()->screen_num], ARG(1,number));
  else
    win = group_find_window (&rp_current_group[current_screen()->screen_num]->mapped_windows, current_window());

  /* Make the switch. */
  if ( new_number >= 0 && win)
    {
      /* Find other window with same number and give it old number. */
      other_win = group_find_window_by_number (rp_current_group[current_screen()->screen_num], new_number);
      if (other_win != NULL)
        {
          ...
          group_resort_window (rp_current_group[current_screen()->screen_num], other_win);
        }
      else
        {
          numset_release (rp_current_group[current_screen()->screen_num]->numset, win->number);
        }

      win->number = new_number;
      numset_add_num (rp_current_group[current_screen()->screen_num]->numset, new_number);

      /* resort the the window in the list */
      group_resort_window (rp_current_group[current_screen()->screen_num], win);
  ...
  if (current_window() != NULL)
    {
      ...
      win_elem = group_find_window (&rp_current_group[win->scr->screen_num]->mapped_windows, win);
      if (!win_elem)
        win_elem = group_find_window (&rp_current_group[win->scr->screen_num]->unmapped_windows, win);
      if (!win_elem)
        {
...
cmd_gnumber (int interactive UNUSED, struct cmdarg **args)
{
  ...
  if (args[1])
    ...
    g = rp_current_group[screen->screen_num];
...
cmd_grename (int interactive UNUSED, struct cmdarg **args)
{
  ...
  group_rename (rp_current_group[current_screen()->screen_num], ARG_STRING(0));
...
{
  groups_merge (ARG(0,group), rp_current_group[current_screen()->screen_num]);
  return cmdret_new (RET_SUCCESS, NULL);
}
...
cmd_gdelete (int interactive UNUSED, struct cmdarg **args)
{
  ...
  if (args[0] == NULL)
    g = rp_current_group[current_screen()->screen_num];
  else
    g = ARG(0,group);
...
cmd_cnext (int interactive, struct cmdarg **args)
{
  ...
  last = group_prev_window (rp_current_group[current_screen()->screen_num], cur);

  if (last)
    for (win = group_next_window (rp_current_group[current_screen()->screen_num], cur);
         win;
         win = group_next_window (rp_current_group[current_screen()->screen_num], win))
      {
        if (win->res_class
...
cmd_cprev (int interactive, struct cmdarg **args)
{
  ...
  /* CUR !in cycle list, so LAST marks last node. */
  last = group_next_window (rp_current_group[current_screen()->screen_num], cur);

  if (last)
    for (win = group_prev_window (rp_current_group[current_screen()->screen_num], cur);
         win;
         win = group_prev_window (rp_current_group[current_screen()->screen_num], win))
      {
        if (win->res_class
            && strcmp (cur->res_class, win->res_class))
          {
...
cmd_cother (int interactive UNUSED, struct cmdarg **args UNUSED)
{
  ...
  cur = current_window();
  if (cur)
    w = group_last_window_by_class (rp_current_group[current_screen()->screen_num], cur->res_class);
...
cmd_iother (int interactive UNUSED, struct cmdarg **args UNUSED)
{
  ...
  cur = current_window();
  if (cur)
    w = group_last_window_by_class_complement (rp_current_group[current_screen()->screen_num], cur->res_class);
...
cmd_vdump (int interactive, struct cmdarg **args)
{
  ...
  list_for_each_entry (cur, &rp_virtuals[current_screen()->screen_num], node) {
...
cmd_vselect (int interactive, struct cmdarg **args)
{
  ...
  if (rp_current_virtuals[screen->screen_num]->number == which)
    /* don't bother */
    return cmdret_new (RET_SUCCESS, NULL);
  ...
  rp_current_virtuals[screen->screen_num]->fconfig = fdump (screen);

  list_for_each_entry (cur, &rp_virtuals[screen->screen_num], node) {
    if (cur->number == which) {
      if (which == 1)
        input = xstrdup ("default");
      else
      ...
      rp_current_virtuals[screen->screen_num] = cur;
