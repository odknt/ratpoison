  ...
  list_for_each_entry (cur, &rp_groups[screen->xine_screen_num], node)
    {
      ...
      if (cur == rp_current_group[screen->xine_screen_num])
      ...
      /* Only put the delimiter between the group, and not after the the last
         group. */
      if (delim && cur->node.next != &rp_groups[screen->xine_screen_num])
...
group_add_new_group (rp_screen *screen, char *name)
{
  ...
  list_add_tail (&g->node, &rp_groups[screen->xine_screen_num]);
...
{
  rp_group *cur;
  struct list_head *last = &rp_groups[screen->xine_screen_num];
...
{
  return list_next_entry (rp_current_group[screen->xine_screen_num], &rp_groups[screen->xine_screen_num], node);
}

rp_group *
group_prev_group (rp_screen *screen)
{
  return list_prev_entry (rp_current_group[screen->xine_screen_num], &rp_groups[screen->xine_screen_num], node);
}
...
group_last_group (rp_screen *screen)
{
  ...
  list_for_each_entry (cur, &rp_groups[screen->xine_screen_num], node)
    {
      if (cur != rp_current_group[screen->xine_screen_num] && cur->last_access > last_access) {
        most_recent = cur;
...
groups_find_group_by_window (rp_window *win)
{
  ...
  list_for_each_entry (cur, &rp_groups[win->scr->xine_screen_num], node)
    {
...
{
  if (rp_current_group[screen->xine_screen_num] == g || g == NULL)
    return;

  set_current_group_1 (screen->xine_screen_num, g);
...
group_delete_group (rp_screen *screen, rp_group *g)
{
  if (list_empty (&(g->mapped_windows))
      && list_empty (&(g->unmapped_windows)))
    {
      ...
      if (list_size (&rp_groups[screen->xine_screen_num]) == 1)
        return GROUP_DELETE_LAST_GROUP;

      /* we can safely delete the group */
      if (g == rp_current_group[screen->xine_screen_num])
        {
